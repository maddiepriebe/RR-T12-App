...
function parseSummaryTable(fullText: string): { rows: SummaryRow[]; expectedCount: number } {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // In the detailed comp table the data line is BEFORE the rank (name → data → address → rank).
    // Scan backward first; scan forward only for the year (which follows rank) and as a fallback
    // for data if backward scan fails. Stop forward scan at the next rank number.
    let dataLine: string | null = null;
    let dataLineIdx = i;
    let yearBuilt: number | null = null;

    // Backward: look for data line up to 8 lines before rank
    for (let j = i - 1; j >= Math.max(0, i - 8); j--) {
      const jl = lines[j].trim();
      // Stop at a different rank number (means we crossed into another comp)
    ...
    // Forward: look for year (and data as fallback), stop at next rank
    for (let j = i + 1; j < Math.min(lines.length, i + 12); j++) {
      const jl = lines[j].trim();
      // Stop when we reach the next comp's rank number
      if (STANDALONE_RANK.test(jl) && parseInt(jl, 10) !== rank) break;
      if (!yearBuilt && STANDALONE_YEAR.test(jl)) {
        yearBuilt = parseInt(jl, 10);
      }
      // Only pick up data line in forward direction if backward scan found nothing
      if (
        !dataLine &&
        DATA_LINE_RE.test(jl) &&
        (jl.match(/\$/g) ?? []).length >= 2
      ) {
        dataLine = jl;
        dataLineIdx = j;
    ...
    // Property name: look back from the earlier of rank index or data line index.
    // Pass 1: strict — skip street addresses (house-number lines), only accept "proper" names.
    // Pass 2: fallback — some properties ARE named by their address ("4909 Auburn"); accept any
    //          line with alphabetic content that isn't pure digits, a year, or a $ line.
    const lookFrom = Math.min(i, dataLineIdx);
    let rawName = "";

    // Pass 1: strict (looksLikeName filters out street addresses)
    for (let j = lookFrom - 1; j >= Math.max(0, lookFrom - 15); j--) {
      const prev = lines[j].trim();
      if (!prev) continue;
      if (STANDALONE_RANK.test(prev) && parseInt(prev, 10) !== rank) break;
      if (!looksLikeName(prev)) continue;
      rawName = prev;
      break;
    }

    // Pass 2: if nothing found, accept any alphabetic line (handles "4909 Auburn"-style names)
    if (!rawName) {
      for (let j = lookFrom - 1; j >= Math.max(0, lookFrom - 15); j--) {
        const prev = lines[j].trim();
        if (!prev) continue;
        if (STANDALONE_RANK.test(prev) && parseInt(prev, 10) !== rank) break;
        if (/^\d+$/.test(prev)) continue;        // pure digits
        if (/\$/.test(prev)) continue;           // rent line
        if (STANDALONE_YEAR.test(prev)) continue; // year
        if (!/[A-Za-z]/.test(prev)) continue;    // must have letters
        rawName = prev;
        break;
      }
    }
...
// KEY: use /UNIT\s+BREAKDOWN/i to catch both "UNIT BREAKDOWN" and "UNIT\nBREAKDOWN"
function parsePropertySections(fullText: string): PropertySection[] {
  const normalised = fullText.replace(/UNIT\s+BREAKDOWN/gi, "§§UNIT_BREAKDOWN§§");
  ...
  for (let i = 0; i < parts.length - 1; i++) {
    ...
    // Valid unit mix body: has adjacent $ amounts (rent per unit + rent per SF)
    const hasRows =
      /\$[\d,]+\s+\$\d+\.\d{2}/.test(bodyText) || // space-sep: $2,290 $4.73
      /\$[\d,]+\$\d+\.\d{2}/.test(bodyText) ||     // concat: $2,290$4.73
