...
import {
  isResManFormat, parseResManRentRoll,
  isResManLeaseChargesFormat, parseResManLeaseCharges,
  isResManSimpleFormat, parseResManSimple,
} from "@/lib/resman-parser";
import { isRealPageFormat, parseRealPage } from "@/lib/realpage-parser";
import { isAOGFormat, parseAOG } from "@/lib/aog-parser";
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── Yardi "Rent Roll with Lease Charges" ─────────────────────
    if (isXlsx && (await isYardiFormat(buffer))) {
      const parsed = await parseYardiRentRoll(buffer);
      return NextResponse.json({ success: true, data: parsed, format: "yardi" });
    }

    // ── ResMan Format 1: "Rent Roll with Lease Charges" (Report1) ─
    if (isXlsx && (await isResManLeaseChargesFormat(buffer))) {
      const parsed = await parseResManLeaseCharges(buffer);
      return NextResponse.json({ success: true, data: parsed, format: "yardi" });
    }

    // ── ResMan Format 2: "Rent Roll" simple (Report1) ────────────
    if (isXlsx && (await isResManSimpleFormat(buffer))) {
      const parsed = await parseResManSimple(buffer);
      return NextResponse.json({ success: true, data: parsed, format: "yardi" });
    }

    // ── ResMan Market Survey RedIQ (Bldg-Unit layout) ────────────
    if (isXlsx && (await isResManFormat(buffer))) {
      const parsed = await parseResManRentRoll(buffer);
      return NextResponse.json({ success: true, data: parsed, format: "yardi" });
    }

    // ── RealPage OneSite Rent Roll Detail ────────────────────────
    if (isXlsx && (await isRealPageFormat(buffer))) {
      const parsed = await parseRealPage(buffer);
      return NextResponse.json({ success: true, data: parsed, format: "yardi" });
    }

    // ── AOG Living Rent Roll ──────────────────────────────────────
    if (isXlsx && (await isAOGFormat(buffer))) {
      const parsed = await parseAOG(buffer);
      return NextResponse.json({ success: true, data: parsed, format: "yardi" });
    }
