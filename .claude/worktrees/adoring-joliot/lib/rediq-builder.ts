...
function applyDateFormat(ws: XLSX.WorkSheet, firstDataRow: number, dateCols: number[]): void {
...
function buildRentRollSheet(rr: ParsedRentRoll): XLSX.WorkSheet {
  ...
  // Data rows start at row 8; date cols: LeaseExp=18, MoveIn=21, MoveOut=22
  applyDateFormat(ws, 8, [18, 21, 22]);
  return ws;
}
...
function buildSourceDataSheet(rr: ParsedRentRoll): XLSX.WorkSheet {
  ...
  // Data rows start at row 11; date cols: LeaseExp=7, MoveInDate=9, MoveOutDate=10
  applyDateFormat(ws, 11, [7, 9, 10]);
  return ws;
}
